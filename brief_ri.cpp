...
inline int smoothedSum(const Mat& patch, int y, int x)
{
    ...
    int img_x = 28 + x;
	int ret = 0;
	for ( int i = img_y - HALF_KERNEL; i < img_y + HALF_KERNEL; ++i ){
		for ( int j = img_x - HALF_KERNEL; j < img_x + HALF_KERNEL; ++j ){
			ret += patch.at< uchar >(i, j);
		}
	}
	return ret;
}

static void pixelTests32(const Mat& img, const std::vector<KeyPoint>& keypoints, Mat& descriptors){
    for (int i = 0; i < (int)keypoints.size(); ++i){
        uchar* desc = descriptors.ptr(i);
        const KeyPoint& pt = keypoints[i];
		float angle = -pt.angle;

		RotatedRect rRect = RotatedRect( pt.pt, Size2f( 59, 59 ), angle );
